...
<%
	...
	int totalProduct=ProductDAO.getDAO().productCount("안경");
	...
	//상품리스트 가져오기
	List<ProductDTO> productList=ProductDAO.getDAO().selectCategoryProduct("안경", startRow, endRow);	
...
<jsp:include page="/header.jsp"/>
    ...
    <div class="row">
        <div class="col-xl-12">
          <div class="site-blocks-cover inner-page" style="background-image: url('<%=request.getContextPath() %>/images/glasses_main.jpg'); background-repeat: no-repeat; background-size: cover; background-position: center">
          </div>         
          <div class="custom-border-bottom py-3">
            <div class="container">
              <div class="row">
                <div class="col-md-12 mb-0"><a href="<%=request.getContextPath() %>/index.jsp">Home</a> <span class="mx-2 mb-0">/</span> <strong class="text-black">안경</strong></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="col-md-12 text-center">
         <div class="site-block-27">
			...
			<% for(int i=startPage;i<=endPage;i++) { %>
				<% if(pageNum!=i) { %>
				<li><a href="<%=request.getContextPath() %>/main/productList_glasses.jsp?pageNum=<%=i%>"><%=i %></a></li>
				<%-- 선택된 번호 --%>
				<% } else { %>
